...
public class DeleteAnnouncementService {
    ...
    public String deleteAnnouncement(Long id){
        Announcement announcement = announcementRepository.findById(id).orElseThrow(()->new RuntimeException(messageSource.getMessage("error.announcement.not.found", null, LocaleContextHolder.getLocale())));
