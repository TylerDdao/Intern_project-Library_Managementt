...
public class GetPolicyService {
    ...
    public PolicyResponse getPolicy(String key){
        Policy policy = policyRepository.findByKey(key).orElseThrow(()->new RuntimeException(messageSource.getMessage("error.policy.not.found",null, LocaleContextHolder.getLocale())));
